...
var nicEditorConfig = bkClass.extend({
	buttons : {
		...
		'left' : {name : __('Left Align'), command : 'justifyleft', noActive : true, key: 'l'},
		'center' : {name : __('Center Align'), command : 'justifycenter', noActive : true, key: 'e'},
		'right' : {name : __('Right Align'), command : 'justifyright', noActive : true, key: 'k'},
		'justify' : {name : __('Justify Align'), command : 'justifyfull', noActive : true, key: 'j'},
		...
		'hr' : {name : __('Horizontal Rule'), command : 'insertHorizontalRule', noActive : true, key: 'h'}
	},
...
var nicEditor = bkClass.extend({
	construct : function(o) {
		...
		bkLib.addEvent(document.body,'click', this.selectCheck.closureListener(this) );
	},
	
	panelInstance : function(e,o) {
		e = this.checkReplace($BK(e));
		var panelElm = new bkElement('section').addClass(NAME+'-header').appendBefore(e);
	...
	getIcon : function(iconName,options){
		return (this.options.iconList[iconName]) ? this.options.iconList[iconName] : '';
	},
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var editorElm = new bkElement('section').setStyle({minHeight : newY+'px'}).addClass(NAME+'-editor').appendBefore(e);
	...
	init : function() {
		...
		this.elm
			.addEvent('click',this.selected.closureListener(this))
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	init : function() {	
		...
		this.elmFrame = new bkElement('iframe').setAttributes({'src' : 'javascript:;', 'frameBorder' : 0, 'allowTransparency' : 'true', 'scrolling' : 'no'}).setStyle({height: '100px', width: '100%'}).addClass(NICE+'-frame').appendTo(this.elm);
	...
	initFrame : function() {
		...
		this.heightUpdate();
		this.frameDoc.addEvent('click', this.selected.closureListener(this)).addEvent('keyup',this.heightUpdate.closureListener(this)).addEvent('keydown',this.keyDown.closureListener(this)).addEvent('keyup',this.selected.closure(this));
		this.ne.fireEvent('add',this);
	},
...
var nicEditorPanel = bkClass.extend({
	construct : function(e,options,nicEditor) {
		...
		this.panelElm = new bkElement('ul').addClass('buttons').appendTo(e);
...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('li').appendTo(e);
		this.button = new bkElement('i').addClass(this.ne.getIcon(buttonName,options)).appendTo(this.contain);
		this.button.addEvent('click', this.mouseClick.closure(this)).noSelect();
	...
	updateState : function() {
		this.contain.removeClass('active');
		if(this.isActive) { this.contain.addClass('active'); }
	},
	...
	disable : function(ins,t) {		
		...
		this.contain.removeClass('enabled');
		this.updateState();	
	},
	...
	mouseClick : function() {
		...
		if(this.options.command) {
			...
			if(!this.options.noActive) { // noActive is set false or undefined
				this.toggleActive();
			}
		}
	...
	key : function(nicInstance,e) {
		comsole.log(his.options.key+" && "+e.ctrlKey+" && "+String.fromCharCode(e.keyCode || e.charCode).toLowerCase()+" == "+this.options.key);
...
var nicEditorPane = bkClass.extend({
	construct : function(elm,nicEditor,options,openButton) {
		...
		this.pane = new bkElement('div').setStyle({fontSize : '12px', border : '1px solid #ccc', 'overflow': 'hidden', padding : '4px', textAlign: 'left', backgroundColor : '#ffffc9'}).addClass(NAME+'-pane').setStyle(options).appendTo(this.contain);
		
		if(openButton && !openButton.options.noClose) {
			this.close = new bkElement('div').setStyle({'float' : 'right', height: '16px', width : '16px', cursor : 'pointer'}).setStyle(this.ne.getIcon('close',nicPaneOptions)).addEvent('click',openButton.removePane.closure(this)).appendTo(this.pane);
		}
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('div').setStyle({width: '90px', height : '20px', cursor : 'pointer', overflow: 'hidden'}).addClass(NAME+'-selectContain').addEvent('click',this.toggle.closure(this)).appendTo(this.margin);
		this.items = new bkElement('div').setStyle({overflow : 'hidden', zoom : 1, border: '1px solid #ccc', paddingLeft : '3px', backgroundColor : '#fff'}).appendTo(this.contain);
		this.control = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'right', height: '18px', width : '16px'}).addClass(NAME+'-selectControl').setStyle(this.ne.getIcon('arrow',options)).appendTo(this.items);
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '66px', height : '14px', marginTop : '1px', fontFamily : 'sans-serif', textAlign : 'center', fontSize : '12px'}).addClass(NAME+'-selectTxt').appendTo(this.items);
